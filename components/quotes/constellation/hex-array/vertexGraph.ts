 ...
 */
function getHexCorners(centerX: number, centerY: number, size: number): Array<{ x: number; y: number }> {
 ...
 */
function areHexNeighbors(t1: HexTile, t2: HexTile): boolean {
  const dx = t2.x - t1.x;
  const dy = t2.y - t1.y;
 ...
 */
export function isVertexInActiveTile(graph: VertexGraph, vertexId: string): boolean {
