...
## Migration from pre-overhaul names

The config-system overhaul renamed several variables. Old `.env` files (e.g. the production server's) must be migrated before deploying. (This section absorbed the former `ENV_MIGRATION.md`.)

### Automated

```bash
python3 scripts/migrate-env.py .env --dry-run          # preview
python3 scripts/migrate-env.py .env                    # in-place (creates .env.backup)
python3 scripts/migrate-env.py .env --output .env.new  # write to a new file
```
...
Unchanged: `NEXT_PUBLIC_SITE_URL`, `NEXT_PUBLIC_DIRECTUS_URL`, `DIRECTUS_KEY`/`DIRECTUS_SECRET`, all `SMTP_*`, `NEXT_PUBLIC_MATOMO_*`, `MATOMO_DB_*`, `CF_ACCESS_*`, `DB_*`, `ADMIN_SESSION_SECRET`.

New variables introduced by the overhaul (add if missing): `ADMIN_PASSCODE`, `ADMIN_SESSION_SECRET`, `ADMIN_REQUIRE_TAILSCALE` (default `false`) — see [ADMIN_ACCESS.md](./ADMIN_ACCESS.md).

Once the production `.env` has been migrated and the site is deployed, this section and `scripts/migrate-env.py` can be retired.
